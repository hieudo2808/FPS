...
namespace FPS.Tests
{
    public class AIPandemoniumTDDTests
    {
        ...
        [Test]
        public void PlayerCombatTelemetry_OnlyOwnerCanReportWeaponState()
        {
            var method = typeof(PlayerCombatTelemetry).GetMethod(
                "ReportWeaponStateServerRpc",
                BindingFlags.Instance | BindingFlags.NonPublic);

            Assert.NotNull(method, "Telemetry should keep a private server RPC boundary.");
            Assert.Null(method.GetCustomAttribute<ServerRpcAttribute>(),
                "Telemetry should not use obsolete RequireOwnership=false ServerRpc.");

            var rpc = method.GetCustomAttribute<RpcAttribute>();
            Assert.NotNull(rpc, "Telemetry reports must use the modern Rpc attribute.");
        }
        ...
        {
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Prefabs/Players/Player.prefab");
        ...
        {
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Prefabs/Screamer/Prefabs/BookHeadMonster_withBlood.prefab");

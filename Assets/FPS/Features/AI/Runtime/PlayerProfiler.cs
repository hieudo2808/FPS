...
namespace FPS
{
    ...
    public class PlayerProfile
    {
        ...
        public SessionPlayerId stablePlayerId;
    ...
    public class PlayerProfiler : MonoBehaviour
    {
        ...
        private readonly Dictionary<ulong, PlayerProfile> profilesByStableId = new Dictionary<ulong, PlayerProfile>();
        ...
        private void RefreshNetworkPlayers()
        {
            ...
            foreach (var client in connectedClientCache)
            {
                ...
                SessionPlayerId stablePlayerId = playerHealth != null ? playerHealth.StablePlayerId : default;
                PlayerProfile profile = null;
                if (stablePlayerId.IsValid)
                    profilesByStableId.TryGetValue(stablePlayerId.Value, out profile);
                if (profile == null)
                    profilesByClientId.TryGetValue(client.ClientId, out profile);
                if (profile == null)
                {
                ...
                if (stablePlayerId.IsValid)
                    profilesByStableId[stablePlayerId.Value] = profile;
        ...
        public void ReportKill(int playerIndex, Vector3 zombiePosition, bool wasHeadshot, float reactionTime)
        {
            ...
            SessionPlayerId stableId = profile.cachedHealth != null
                ? profile.cachedHealth.StablePlayerId
                : default;
            NetworkManager networkManager = NetworkManager.Singleton;
            NetworkGameManager.Instance?.Telemetry?.RecordKill(
                stableId,
        ...
        public PlayerProfile GetProfileByStableId(SessionPlayerId playerId)
        {
            return playerId.IsValid && profilesByStableId.TryGetValue(playerId.Value, out PlayerProfile profile)
                ? profile
                : null;
        }
